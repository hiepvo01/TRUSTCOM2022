...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=100, n_batch=64):
	...
	for i in range(n_steps):
		...
		[X_fake, labels_fake, labels_other], y_fake = generate_fake_samples(g_model, latent_dim, half_batch)
		# update discriminator model weights
		_,d_f,d_f2,d_f3 = d_model.train_on_batch(X_fake, [y_fake, labels_fake, labels_other])
		# prepare points in latent space as input for the generator
		[z_input, z_labels] = generate_latent_points(latent_dim, n_batch)
		# create inverted labels for the fake samples
		y_gan = ones((n_batch, 1))
		# update the generator via the discriminator's error
		_,g_1,g_2,g_3 = gan_model.train_on_batch([z_input, z_labels], [y_gan, z_labels], [])
